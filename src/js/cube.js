var colorss = ['blue', 'green', 'white', 'yellow', 'orange', 'red'],
...
function assembleCube() {
  function moveto(face) {
    ...
    pieces[i].children[face]
      ...
      .setAttribute('class', 'sticker ' + colorss[face]);
...
function mousedown(md_e) {
  startMouseX = md_e.clientX;
  startMouseY = md_e.clientY;
  var startXY = pivot.style.transform.match(/-?\d+\.?\d*/g).map(Number),
    element = md_e.target.closest('.element'),
    face = [].indexOf.call((element || cube).parentNode.children, element);
  ...
  function mousemove(mm_e) {
    if (element) {
      var gid = /\d/.exec(
        document.elementFromPoint(mm_e.clientX, mm_e.clientY).id
      );
      if (gid && gid.input.includes('anchor')) {
        mouseup();
        var e =
      ...
      pivot.style.transform =
        'rotateX(' +
        (startXY[0] - (mm_e.clientY - startMouseY) / 2) +
        'deg)' +
        'rotateY(' +
        (startXY[1] + (mm_e.clientX - startMouseX) / 2) +
        'deg)';
    }
  }
